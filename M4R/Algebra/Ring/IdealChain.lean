...
namespace M4R
  ...
  namespace Ideal
    ...
    namespace chain
      ...
      noncomputable def strict_increasing_index [Ring α] {c : chain α} (h : ∀ N, ∃ (n : Nat) (hn : N ≤ n), c n ≠ c N) : Nat → Nat
      | 0   => 0
      | n+1 => choose (h (strict_increasing_index h n))
      theorem strict_increasing_index.idx_strict [Ring α] {c : chain α} (h : ∀ N, ∃ (n : Nat) (hn : N ≤ n), c n ≠ c N) (n : Nat) :
      ...
      theorem strict_increasing_index.strict [Ring α] {c : chain α} (h : ∀ N, ∃ (n : Nat) (hn : N ≤ n), c n ≠ c N) (n : Nat) :
      ...
      noncomputable def strictified [Ring α] {c : chain α} (h : ∀ N, ∃ (n : Nat) (hn : N ≤ n), c n ≠ c N) : chain α where
        f := fun n => match n with
        | 0   => 1
        | n+1 => c (strict_increasing_index h (if c 0 = 1 then n.succ else n))
      theorem strictified.base [Ring α] {c : chain α} (h : ∀ N, ∃ (n : Nat) (hn : N ≤ n), c n ≠ c N) : (strictified h).base 1 := rfl
      theorem strictified.base1 [Ring α] {c : chain α} (h : ∀ N, ∃ (n : Nat) (hn : N ≤ n), c n ≠ c N) (h0 : c 0 = 1) :
        ∀ n, strictified h n = c (strict_increasing_index h n)
      | 0   => base h ▸ h0.symm
      | n+1 => by simp only [strictified, h0, ite_true]; rfl
      theorem strictified.not_base1 [Ring α] {c : chain α} (h : ∀ N, ∃ (n : Nat) (hn : N ≤ n), c n ≠ c N) (h0 : c 0 ≠ 1) (n : Nat) :
        strictified h n.succ = c (strict_increasing_index h n) := by simp only [strictified, h0, ite_false]
      theorem strictified.descending [Ring α] {c : chain α} (h : ∀ N, ∃ (n : Nat) (hn : N ≤ n), c n ≠ c N) (hc : c.descending) : (strictified h).descending :=
        fun n => by
          byCases h0 : c 0 = 1
      ...
      theorem strictified.strict_infinite [Ring α] {c : chain α} (h : ∀ N, ∃ (n : Nat) (hn : N ≤ n), c n ≠ c N) : (strictified h).strict_infinite :=
        fun n => by
          byCases h0 : c 0 = 1
