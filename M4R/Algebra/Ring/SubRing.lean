...
namespace M4R
  ...
  namespace SubRing
    ...
    protected def image [NCRing α] (s : SubRing α) (a : α) (p : a ∈ s.subset) : ↑s.subset := ⟨a, p⟩
    protected theorem image_eq [NCRing α] (s : SubRing α) (a b : ↑s.subset) :
      a = b ↔ Set.inclusion a = Set.inclusion b :=
        ⟨congrArg Set.inclusion, Set.elementExt⟩
        
    instance SubRingRing [NCRing α] (s : SubRing α) : NCRing ↑s.subset where
